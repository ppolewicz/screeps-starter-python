...
class CarrySource:
    ...
    def _get_closest_energetic_container(cls, creep):
        ...
        source_filter = lambda s: (
            s.structureType == STRUCTURE_CONTAINER and s.store[RESOURCE_ENERGY] >= free_capacity
        )
        return creep.pos.findClosestByRange(FIND_STRUCTURES, filter=source_filter)
    ...
    @classmethod
    def get_source(cls, creep):
        source = cls._get_cached_source(creep)
        if source:
            return source
        for source_getter in cls._get_source_getters(creep):
            source = source_getter(creep)
            if source:
    ...
    def _get_fullest_miner_container(cls, creep):
        containers = []
        for s in creep.room.find(FIND_MY_STRUCTURES):
